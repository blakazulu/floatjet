  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (issues.length > 0) {
        ...
        schemaStats.issues.push(...issues.map(i => ({
          path: relativePath,
          issue: i
        })));
      }
...
console.log(`Articles with Article schema: ${schemaStats.hasArticleSchema}/${schemaStats.totalArticles}`);
console.log(`Articles with FAQ schema: ${schemaStats.hasFAQSchema}/${schemaStats.totalArticles}`);
console.log(`Articles with Breadcrumb schema: ${schemaStats.hasBreadcrumbSchema}/${schemaStats.totalArticles}`);
console.log(`Articles missing featured image: ${schemaStats.missingImage}/${schemaStats.totalArticles}\n`);
...
if (schemaStats.hasFAQSchema < schemaStats.totalArticles * 0.5) {
  ...
  recommendations.push(`Only ${pct}% of articles have FAQ sections (consider adding more for rich snippets)`);
}

if (schemaStats.missingImage > 0) {
  recommendations.push(`${schemaStats.missingImage} articles missing featured images (reduces Article schema quality)`);
}
